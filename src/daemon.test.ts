  ...
  private async handle(message: any): Promise<void> {
    ...
    try {
      ...
      if (action === "requestApproval") {
        return this.send({ type: "response", id, result: { approved: true } });
      }
      if (action === "cdp") {
...
describe("DaemonServer + DaemonClient", () => {
  ...
  it("includes requester identity in relayed approval descriptions", async () => {
    const seen: string[] = [];
    ...
    await a.connect();
    await a.request({ action: "requestApproval", description: "Allow click: buy now" });
    expect(seen[0]).toContain("agent-x");
    expect(seen[0]).toContain("buy now");
  });
});
